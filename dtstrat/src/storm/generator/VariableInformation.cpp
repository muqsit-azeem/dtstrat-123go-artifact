...
VariableInformation::VariableInformation(storm::prism::Program const& program, uint64_t reservedBitsForUnboundedVariables, bool outOfBoundsState)
    ...
    for (auto const& booleanVariable : program.getGlobalBooleanVariables()) {
        ...
        ++totalBitOffset;
        stateVariableModuleAssociation.push_back(-1);
    }
    ...
    for (auto const& module : program.getModules()) {
        for (auto const& booleanVariable : module.getBooleanVariables()) {
            ...
            ++totalBitOffset;
            stateVariableModuleAssociation.push_back(counter);
        }
...
std::vector<int> VariableInformation::getStateVariableModuleAssociation() const {
    return stateVariableModuleAssociation;
}
