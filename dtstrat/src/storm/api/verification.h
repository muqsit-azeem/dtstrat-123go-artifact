...
//
// Verify with DTStrat exploration engine
//
template<typename ValueType>
typename std::enable_if<std::is_same<ValueType, double>::value, std::unique_ptr<storm::modelchecker::CheckResult>>::type verifyWithDTStratExplorationEngine(
    storm::Environment const& env, storm::storage::SymbolicModelDescription const& model,
    storm::modelchecker::CheckTask<storm::logic::Formula, ValueType> const& task) {
    STORM_LOG_THROW(model.isPrismProgram(), storm::exceptions::NotSupportedException, "Exploration engine is currently only applicable to PRISM models.");
    storm::prism::Program const& program = model.asPrismProgram();
    ...
    std::unique_ptr<storm::modelchecker::CheckResult> result;
    if (program.getModelType() == storm::prism::Program::ModelType::DTMC) {
        storm::modelchecker::SparseExplorationDTStratModelChecker<storm::models::sparse::Dtmc<ValueType>> checker(program);
        if (checker.canHandle(task)) {
            result = checker.check(env, task);
        }
    } else if (program.getModelType() == storm::prism::Program::ModelType::MDP) {
        storm::modelchecker::SparseExplorationDTStratModelChecker<storm::models::sparse::Mdp<ValueType>> checker(program);
        if (checker.canHandle(task)) {
            result = checker.check(env, task);
        }
    } else {
...
template<typename ValueType>
std::unique_ptr<storm::modelchecker::CheckResult> verifyWithDTStratExplorationEngine(storm::storage::SymbolicModelDescription const& model,
                                                                              storm::modelchecker::CheckTask<storm::logic::Formula, ValueType> const& task) {
    Environment env;
    return verifyWithDTStratExplorationEngine(env, model, task);
}

//
// Verify with Learning engine
//
template<typename ValueType>
typename std::enable_if<std::is_same<ValueType, double>::value, std::unique_ptr<storm::modelchecker::CheckResult>>::type verifyWithLearningEngine(
    storm::Environment const& env, storm::storage::SymbolicModelDescription const& model,
    storm::modelchecker::CheckTask<storm::logic::Formula, ValueType> const& task,
    ...
    std::unique_ptr<storm::modelchecker::CheckResult> result;
    if (program.getModelType() == storm::prism::Program::ModelType::DTMC) {
        storm::modelchecker::LearningStrategiesModelChecker<storm::models::sparse::Dtmc<ValueType>> checker(program, properties, preprocessedProperties);
        if (checker.canHandle(task)) {
            result = checker.check(env, task);
        }
    } else if (program.getModelType() == storm::prism::Program::ModelType::MDP) {
        storm::modelchecker::LearningStrategiesModelChecker<storm::models::sparse::Mdp<ValueType>> checker(program, properties, preprocessedProperties);
        if (checker.canHandle(task)) {
            result = checker.check(env, task);
        }
    } else {
...
template<typename ValueType>
typename std::enable_if<!std::is_same<ValueType, double>::value, std::unique_ptr<storm::modelchecker::CheckResult>>::type verifyWithLearningEngine(
    storm::Environment const&, storm::storage::SymbolicModelDescription const&, storm::modelchecker::CheckTask<storm::logic::Formula, ValueType> const&, std::vector<storm::jani::Property>, boost::optional<std::vector<storm::jani::Property>>) {
    STORM_LOG_THROW(false, storm::exceptions::NotSupportedException, "Exploration engine does not support data type.");
}

template<typename ValueType>
std::unique_ptr<storm::modelchecker::CheckResult> verifyWithLearningEngine(storm::storage::SymbolicModelDescription const& model,
                                                                           storm::modelchecker::CheckTask<storm::logic::Formula, ValueType> const& task,
                                                                           std::vector<storm::jani::Property> properties,
                                                                           boost::optional<std::vector<storm::jani::Property>> preprocessedProperties) {
    Environment env;
    return verifyWithLearningEngine(env, model, task, properties, preprocessedProperties);
}
