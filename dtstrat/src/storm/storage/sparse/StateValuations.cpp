...
StateValuations::StateValuation StateValuations::getStateValuation(uint const& currentStateId) const {
...
StateValuations::StateValuation  StateValuationsBuilder::getStateValuation(const uint& currentStateId) const {
    return currentStateValuations.getValuation(currentStateId);
}
...
int StateValuations::StateValuation::operator[](uint index) const {
    STORM_LOG_THROW(rationalValues.empty(),exceptions::NotSupportedException,"Only models with integer/boolean state variables are supported.");
    STORM_LOG_ASSERT(index<booleanValues.size()+integerValues.size()+rationalValues.size(),"Index of StateValuation is too big.");
    if (index<booleanValues.size()) {
        return booleanValues[index];
    } else {
        return integerValues[index-booleanValues.size()];
    }
}

arma::Col<int> StateValuations::StateValuation::transform() {
    std::vector<int> values;
    for (auto v : this->booleanValues) {
        values.push_back(v);
    }
    for (auto v: this->integerValues) {
        values.push_back(v);
    }
    return values;
}

arma::Col<int> StateValuations::StateValuation::transform() const {
    std::vector<int> values;
    for (auto v : this->booleanValues) {
        values.push_back(v);
    }
...
uint StateValuations::StateValuation::size() const {
    return booleanValues.size() + integerValues.size() + rationalValues.size();
}

std::ostream& operator<<(std::ostream& os, const StateValuations::StateValuation& valuation) {
    for (uint i=0; i<valuation.size(); i++) os << valuation[i] << " ";
    return os;
}
